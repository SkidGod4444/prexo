...
auditLogs.get("/:projectId/all", async (c) => {
  ...
  if (!projectId) {
    ...
    return c.json({ message: "Failed to fetch logs", error: error instanceof Error ? error.message : error }, 500);
  }
});

auditLogs.post("/audit/create", async (c) => {
    const { time, actor, action, endpoint, credits, projectId } = await c.req.json();
    if (!projectId) {
...
auditLogs.post("/usage/create", async (c) => {
  const { year, month, api_calls, credits_used, projectId } = await c.req.json();
  if (!projectId) {
    return c.json({ error: "projectId is required" }, 400);
  }
