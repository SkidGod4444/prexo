...
function DialogBackdrop({
  ...
  return (
    <DialogPrimitive.Backdrop
      ...
      className={cn(
        "fixed inset-0 z-50 bg-secondary/16 backdrop-blur-sm transition-all duration-200 data-ending-style:opacity-0 data-starting-style:opacity-0",
        className,
      )}
      {...props}
    />
...
function DialogFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex flex-col-reverse gap-2 sm:-mx-6 sm:mt-2 sm:-mb-6 sm:flex-row sm:justify-end sm:rounded-b-xl sm:border-t sm:bg-background/50 sm:px-6 sm:py-4",
        className,
      )}
      {...props}
    />
